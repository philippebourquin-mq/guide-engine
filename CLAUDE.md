...
## Débordement horizontal mobile — deux causes distinctes, deux correctifs

Le symptôme « ça déborde à droite, obligé de re-zoomer » avait en réalité **deux origines séparées**, découvertes l'une après l'autre — ne pas se contenter de la première trouvée si le symptôme persiste après son correctif :

1. **iOS Safari zoome au focus d'un champ < 16px.** La quasi-totalité des champs/`contenteditable` de l'admin sont à 12-14px (pour rester discrets visuellement) — taper dans n'importe quel titre/texte déclenchait le zoom automatique de Safari, qui restait ensuite « collé » même après avoir quitté le champ. Corrigé dans `admin.html` par une media query `@media (max-width:767px) { input, textarea, [contenteditable="true"] { font-size:16px !important; } }` — `!important` nécessaire pour passer devant les tailles inline. Desktop inchangé (règle bornée à mobile). Effet de bord : un titre de section long (ex. « Photos » sur une carte album avec le badge « Album » qui grignote la largeur dispo) pouvait passer en 2 lignes à 16px — `h2[data-field="section-title"]` a donc aussi reçu `white-space:nowrap;overflow:hidden;text-overflow:ellipsis`.
2. **`min-width:auto` implicite sur les enfants de grille.** Une chaîne non coupable (URL collée sans espace, mot très long) dans un item force sa colonne à s'élargir jusqu'à la largeur intrinsèque du texte — même déclarée `1fr`, même avec la media query mobile qui réduit `.guide-grid-3`/`.cyc-row` à une seule colonne (celle-ci ne change que le *nombre* de colonnes, pas ce comportement). `overflow-x:hidden` sur `html,body` (déjà en place) masque le symptôme visuellement mais ne corrige rien — le texte reste coupé/clippé au lieu de s'enrouler proprement. Vrai correctif dans `engine.css` : `.guide-grid-3 > *, .cyc-row > * { min-width: 0; }` + `overflow-wrap: break-word` sur les `p`/`h3` de ces grilles. S'applique aussi à l'album (`.album-section` porte déjà la classe `guide-grid-3`).
3. **Piège découvert en creusant #2** : `admin.html` a sa propre fonction `classicMirrorHtml()` qui **duplique** la logique de grille de `renderClassicSection()` (nécessaire pour les hooks de décoration par item) au lieu de la réutiliser — elle avait donc son propre `style="display:grid;grid-template-columns:repeat(3,1fr)..."` inline, jamais mis à jour lors du premier passage sur le responsive mobile (qui n'avait touché que `engine.js`). Corrigée pour utiliser les mêmes classes `guide-grid-3`/`cyc-row`/`cyc-stack` que le moteur. **À chaque évolution du système de grille dans `engine.js`, vérifier aussi `classicMirrorHtml()` dans `admin.html`** — c'est le seul endroit qui réimplémente cette logique au lieu de l'importer.
...
Chaque ligne : rond de couleur (clic → nuancier inline, même mécanique que la création — `changeSiteColor`), label + `owner/repo` cliquables (`data-open-site`, ouvre `https://{owner}.github.io/{repo}/` dans un nouvel onglet), icône corbeille. Le clic sur toute la ligne (hors rond de couleur et corbeille, qui stoppent la propagation) ouvre le site publié — usage visé : accéder rapidement au site en direct pour l'ajouter à l'écran d'accueil mobile (partage navigateur → « Sur l'écran d'accueil »), pas de proposer un install prompt programmatique (impossible côté iOS Safari, pas d'API pour ça).
...
- **Aucun changement côté `engine.js`/rendu du site publié** : un chemin relatif (`./photos/xxx.jpg`) fonctionne exactement comme une data URI dans un `<img src>` — le moteur ne sait même pas que ça a changé. **`index.html` et `photos/` sont dans le même dépôt, donc la même origine — le chemin relatif s'y résout correctement.**
...
**Piège découvert après coup, sur Roadtrip 26 : les photos migrées s'affichaient cassées dans l'admin** (mais correctement sur le site publié). Cause : `admin.html` vit sur l'origine de `guide-engine` — un dépôt différent de celui de la destination éditée. Un chemin relatif tel quel dans le mirror (`<img src="./photos/xxx.jpg">`) se résout donc contre `guide-engine` (404) au lieu du dépôt de la destination. Corrigé par une paire de fonctions symétriques :
- `resolvePhotoUrl(cfg, src)` / `resolvePhotosInData(data, cfg)` — appelée dans `renderAdmin(data)`, réécrit tout chemin relatif en URL absolue `https://{owner}.github.io/{repo}/...` avant de peupler `card._items`/`_albumPhotos`/`_metaState.coverImage`. `data:`/`http(s)` déjà absolus, laissés intacts.
- `relativizePhotoUrl(cfg, src)` — appelée en fin de `collectData()`, fait l'inverse : ne relativise que ce qui commence par le préfixe github.io **de la destination active**, pour que `data.json` publié garde des chemins portables (fonctionnant aussi en dev local, pas seulement sur Pages).
- **Piège dans le piège** : la branche album de `collectData()` retournait `card._albumPhotos` **par référence** (pas une copie) — relativiser in-place aurait corrompu l'état d'édition en cours (les URL absolues de travail seraient devenues relatives, cassant l'affichage au rerender suivant). Corrigé en copiant (`.map(p => ({...p}))`) avant de relativiser, comme le fait déjà la branche classique via son propre `.map()`. **Toute nouvelle transformation dans `collectData()` doit rester sur une copie, jamais sur `card._items`/`card._albumPhotos` eux-mêmes** — ce sont aussi l'état d'édition live, pas seulement la source du payload de publication.
...
**Axe dominant** : une seule poignée, au coin — un drag pour « juste élargir » part quasi toujours un peu en diagonale (l'œil suit le coin), et sur une photo à peu près carrée `colUnit≈rowUnit`, ce qui pouvait faire passer les lignes en même temps que les colonnes (`1x1→1x2` se retrouvait forcé en `2x2`, signalé après coup). `move()` ne fait bouger un axe seul que si son déplacement (`dx/colUnit` ou `dy/rowUnit`) est nettement dominant (≥1.6×) sur l'autre ; sinon (geste franchement diagonal) les deux bougent ensemble, ce qui reste voulu. Vérifié par simulation de drag (PointerEvent) : horizontal pur et horizontal-avec-jitter (jusqu'à ~90px de dérive verticale sur une unité de 220px) restent tous deux `1x2` ; un drag en diagonale égale donne bien `2x2` ; vertical pur donne `2x1`. **Ne pas revenir à un calcul indépendant par axe** sans ce garde-fou — régression silencieuse, aucune erreur, juste des formats qui semblent inatteignables.
